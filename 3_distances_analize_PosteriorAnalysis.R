...
reference_uzskaites <- read_excel("./Dati/originalie_dati.xlsx", sheet = "Uzskaites")
...
sc <- as.data.frame(TudfGDS@yearlySiteCovs)
m_J <- mean(sc$Jday, na.rm = TRUE)
s_J <- sd(sc$Jday, na.rm = TRUE)
m_T <- mean(sc$temp_vid, na.rm = TRUE); s_T <- sd(sc$temp_vid, na.rm = TRUE)
m_A <- mean(sc$apg_vid, na.rm = TRUE); s_A <- sd(sc$apg_vid, na.rm = TRUE)
sc$Jday_sc <- (sc$Jday - m_J) / s_J
sc$temp_sc <- (sc$temp_vid - m_T) / s_T
sc$apg_sc  <- (sc$apg_vid  - m_A) / s_A
TudfGDS2 <- TudfGDS
yearlySiteCovs(TudfGDS2) <- sc

m_Pollard_haz2<- gdistsamp(~1, # lambda = abundance
                          ~1+I(Jday_sc^2)+temp_sc+I(temp_sc^2) + apg_sc, # phi = availability
                          ~1, # pi = detection
                          TudfGDS2, 
                          keyfun ="hazard", 
                          output=Toutput, 
...
summary(m_Pollard_haz2)

Jseq <- seq(153, 244, 5)
nd_G1 <- data.frame(Jday = Jseq, apg_vid = 60,  temp_vid = 13)
nd_G2 <- data.frame(Jday = Jseq, apg_vid = 100, temp_vid = 33)
nd_G1$Jday_sc <- (nd_G1$Jday - m_J) / s_J
nd_G1$temp_sc <- (nd_G1$temp_vid - m_T) / s_T
nd_G1$apg_sc  <- (nd_G1$apg_vid  - m_A) / s_A
nd_G2$Jday_sc <- (nd_G2$Jday - m_J) / s_J
nd_G2$temp_sc <- (nd_G2$temp_vid - m_T) / s_T
nd_G2$apg_sc  <- (nd_G2$apg_vid  - m_A) / s_A

# vcov based
phi_terms <- ~ 1 + I(Jday_sc^2) + Jday_sc + temp_sc + I(temp_sc^2) + apg_sc

get_vcov_phi <- function(fit) {
  bphi <- coef(fit, type = "phi")
  Vphi <- tryCatch(vcov(fit, type = "phi"), error = function(e) NULL)
  
  if (!is.null(Vphi)) {
    # make sure ordering matches bphi
    return(Vphi[names(bphi), names(bphi), drop = FALSE])
  }
  
  # fallback: subset full vcov
  V <- vcov(fit)
  if (!all(names(bphi) %in% colnames(V))) {
    stop("Could not find phi coefficient names inside vcov(fit).")
  }
  V[names(bphi), names(bphi), drop = FALSE]
}

get_X_phi <- function(newdata, bphi) {
  X <- model.matrix(phi_terms, data = newdata)
  
  # Map coef names like "phi(temp_sc)" -> "temp_sc", and "phi(Int)" -> "(Intercept)"
  nm <- names(bphi)
  nm2 <- sub("^phi\\((.*)\\)$", "\\1", nm)   # drop phi(...)
  nm2[nm2 == "Int"] <- "(Intercept)"         # intercept naming
  
  miss <- setdiff(nm2, colnames(X))
  if (length(miss)) {
    stop("Design matrix missing columns after mapping: ",
         paste(miss, collapse = ", "),
         "\nColumns in X are: ", paste(colnames(X), collapse = ", "))
  }
  
  # Reorder columns to match coef order
  X_aligned <- X[, nm2, drop = FALSE]
  
  # Put the original coef names back on X (so downstream code using bphi names works)
  colnames(X_aligned) <- nm
  
  X_aligned
}

contrast_phi_vcov <- function(fit, nd1, nd2) {
  bphi <- coef(fit, type = "phi")
  Vphi <- get_vcov_phi(fit)
  
  X1 <- get_X_phi(nd1, bphi)
  X2 <- get_X_phi(nd2, bphi)
  
  eta1 <- as.numeric(X1 %*% bphi)
  eta2 <- as.numeric(X2 %*% bphi)
  
  DX <- X2 - X1
  se_deta <- sqrt(rowSums((DX %*% Vphi) * DX))
  
  deta <- eta2 - eta1
  deta_lwr <- deta - 1.96 * se_deta
  deta_upr <- deta + 1.96 * se_deta
...
aaa=contrast_phi_vcov(m_Pollard_haz2, nd_G1, nd_G2)

# bootstrapping comparisons
library(MASS)
bootstrap_phi_contrast <- function(fit, nd1, nd2, R = 4000, seed = 1) {
  set.seed(seed)
  ...
  # coefficient draws
  B <- MASS::mvrnorm(n = R, mu = bphi, Sigma = Vphi)  # R x p
  ...
  dphi_draw <- phi2 - phi1
  deta_draw <- eta2 - eta1
  OR_draw   <- exp(deta_draw)
  
  # fitted (point) estimates
  eta1_hat <- as.numeric(X1 %*% bphi)
  eta2_hat <- as.numeric(X2 %*% bphi)
  phi1_hat <- plogis(eta1_hat)
  phi2_hat <- plogis(eta2_hat)
  
  dphi_hat <- phi2_hat - phi1_hat
  deta_hat <- eta2_hat - eta1_hat
  OR_hat   <- exp(deta_hat)
  
  # bootstrap CIs (per Jday)
  dphi_ci <- t(apply(dphi_draw, 2, quantile, probs = c(0.025, 0.975), na.rm = TRUE))
  OR_ci   <- t(apply(OR_draw,   2, quantile, probs = c(0.025, 0.975), na.rm = TRUE))
  deta_ci <- t(apply(deta_draw, 2, quantile, probs = c(0.025, 0.975), na.rm = TRUE))
  ...
  # marginal summaries across Jday
  mean_dphi_draw <- rowMeans(dphi_draw)
  mean_OR_draw   <- rowMeans(OR_draw)
  
  marginal <- list(
    mean_dphi = c(
      est = mean(dphi_hat),
      lwr = unname(quantile(mean_dphi_draw, 0.025)),
      upr = unname(quantile(mean_dphi_draw, 0.975))
    ),
    mean_OR = c(
      est = mean(OR_hat),
      lwr = unname(quantile(mean_OR_draw, 0.025)),
      upr = unname(quantile(mean_OR_draw, 0.975))
    )
  )
  
  list(per_Jday = per_Jday, marginal = marginal)
}

rez <- bootstrap_phi_contrast(m_Pollard_haz2, nd_G1, nd_G2, R = 4000, seed = 1)
reztab=rez$per_Jday
rez$marginal

ggplot(reztab,aes(x=Jday,y=dphi,ymin=dphi_lwr,ymax=dphi_upr))+
